...
DIST_COMMON = README $(am__configure_deps) $(dist_pkgdata_DATA) \
	...
	$(top_srcdir)/win32/installer.iss.in ABOUT-NLS AUTHORS COPYING \
...
CONFIG_CLEAN_FILES = win32/installer.iss
...
LIBINTL = 
LIBOBJS = 
LIBS = -lXtst -lXext -lX11 -lmicrohttpd
...
X_LIBS = 
X_PRE_LIBS =  -lSM -lICE
abs_builddir = /home/raman/RandD/opengazer/eviacam-1.7.0
abs_srcdir = /home/raman/RandD/opengazer/eviacam-1.7.0
...
win32/installer.iss: $(top_builddir)/config.status $(top_srcdir)/win32/installer.iss.in
...
deb-src: configure dist
	...
	cd $(PACKAGE)-$(VERSION) ; \
	cp -a "$(abs_srcdir)/debian" . ; \
	dpkg-buildpackage -S -i -rfakeroot ; \
	cd .. ; \
	$(RM) -r $(PACKAGE)-$(VERSION)
...
deb-src-changes: configure
	...
	cd $(PACKAGE)-$(VERSION) ; \
	cp -a "$(abs_srcdir)/debian" . ; \
	dpkg-buildpackage -S -i -rfakeroot --changes-option=-sd ; \
	cd .. ; \
	$(RM) -r $(PACKAGE)-$(VERSION)
