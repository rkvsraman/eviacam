    ...
    #endif
{
    ...
    // Read screen size
    if(CMouseControl::isInitialized == 0){
        CMouseControl::isInitialized = 1;
        printf("Isinitialized checked once");
...
float CMouseControl::MovePointerRel (float dx, float dy, int* dxRes, int* dyRes)
{
    ...
    if (m_enabledRestrictedWorkingArea && !m_enabledWrapPointer) {
        printf("Entered Restricted work area \n");
...
//HTTP handler
int  CMouseControl::isInitialized  = 0;
long  CMouseControl::xVal  = 0;
long  CMouseControl::yVal  = 0;
int  CMouseControl::flagsVal  = 0;

int CMouseControl::answer_to_connection (void *cls, struct MHD_Connection *connection,
                                         ...
                                         size_t * upload_data_size, void **con_cls)
{
    ...
    char page[100] ;
    sprintf(page,"{ \"x\"=\"%ld\", \"y\"=\"%ld\", \"flags\"=\"%d\"}",xVal,yVal,flagsVal);
...
void CMouseControl::SendMouseCommand (long x, long y, int flags)
{
    ...
    //return;
    CMouseControl::xVal = x;
    CMouseControl::yVal = y;
    CMouseControl::flagsVal = flags;
    //return;
#if defined(WIN32)
    INPUT ip;
